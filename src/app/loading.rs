...
fn load_bytes_with_detection(
    ...
    if is_gif {
        ...
        // If GIF loading fails, fall through to static loading
    }

    // For non-GIF images, check dimensions without full decode
    ...
    // Not large – decode fully as static image
    match crate::loader::load_image_from_bytes(&bytes, path_hint) {
        Ok(img) => Ok(LoadedImage::Static(img)),
        Err(e) => {
            // If we failed but it was a GIF, try full GIF loading
            if is_gif {
