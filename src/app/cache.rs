        ...
        if let Some(image_id) = self.get_image_id(index) {
            ...
            let texture = match &loaded_image {
                LoadedImage::Static(img) => {
                    ...
                    const MAX_CACHE_SIZE: u32 = 18000; // Max dimension
                    const MAX_CACHE_PIXELS: u64 = 250_000_000; // 4x50 megapixels max for cache

                    if width > MAX_CACHE_SIZE || height > MAX_CACHE_SIZE {
                        return; // Don't cache excessively large images
                    }

                    let pixels = width as u64 * height as u64;
                    if pixels > MAX_CACHE_PIXELS {
                        return; // Don't cache images with too many pixels
                    }
